...
class Ventana(tk.Tk):
    def __init__(self):
        ...
        self.ruta_imagen = "img/detras1.jpeg"
    ...
    def procesar_imagen(self, contraste, brillo, gamma):
        ...
        imagen_yuv = cv2.cvtColor(self.imagen, cv2.COLOR_BGR2YUV)
        canal_y = imagen_yuv[:,:,0]

        canal_y_proc = Procesador.contraste_brillo_centrado(canal_y, contraste, brillo)
        canal_y_proc = Procesador.correccion_gamma(canal_y_proc, gamma)

        if self.__check_status.get():
            canal_y_proc = Procesador.ecualizar_hist(canal_y_proc)

        imagen_yuv[:,:,0] = canal_y_proc
        self.imagen_procesada = cv2.cvtColor(imagen_yuv, cv2.COLOR_YUV2BGR)
class Procesador:
    @staticmethod
